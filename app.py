...
# Function to save the validated image in the temp folder
def save_image(file, product_id):
    """Save the image in the temp folder, replacing old image if needed."""
...
def validate_image():
    ...
    # Skip validation for Apple/iPhone images
    ...
    if make and 'Apple' in make:
        ...
        # Existing validation for other images
        if not exif_data:
            print("EXIF data extraction failed.")
            is_valid = False
            validation_message = "No EXIF data found! Validation failed."
...
def get_form_data():
    global json_data
    print('json_data:', json_data)  # This will print out the data sent to the frontend
    return jsonify(json_data)  # json_data should now be properly serialized and sent
